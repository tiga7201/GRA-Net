...
if __name__ == '__main__':
    ...
    # classes = ["_background_", "scrape", "scale"]
    classes = ["_background_", "blowhole", "break", "crack", "fray", "uneven"]
    
    count = os.listdir("./datasets/before/")  # 包含所有数据和标签的列表
    for i in range(0, len(count)):  # 528
        ...
        if os.path.isfile(path) and path.endswith('json'):
            ...
            PIL.Image.fromarray(img).save(osp.join(jpgs_path, count[i].split(".")[0]+'.png'))  # 图片保存为png格式
