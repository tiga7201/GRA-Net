...
from nets.GRA_block import LRM, FIA, ABR, LRMs
from nets.mobile_vit_block import MobileViTBlock
...
class PSPNet(nn.Module):
    def __init__(self, num_classes, downsample_factor, backbone="resnet50", pretrained=True, aux_branch=True):
        ...
        self.master_branch = nn.Sequential(
            # _PSPModule(out_channel, pool_sizes=[1, 2, 3, 6], norm_layer=norm_layer),  # 输出channel=80
            LRM(out_channel),  # #输出channel=320
            # nn.Conv2d(out_channel, out_channel//4, kernel_size=(1, 1)),  # #加一个降维层
            # #nn.Conv2d(out_channel//4, num_classes, kernel_size=(1, 1))
        )

        self.MVit_block1 = MobileViTBlock(dim=aux_channel * 4, depth=3, channel=aux_channel,
                                          kernel_size=3, patch_size=(2, 2), mlp_dim=aux_channel * 6)
        self.conv1 = conv_1x1_bn(aux_channel, 160)
        self.MVit_block2 = MobileViTBlock(dim=160 * 4, depth=1, channel=160,
                                          kernel_size=3, patch_size=(2, 2), mlp_dim=160 * 6)
        self.conv2 = conv_1x1_bn(160, out_channel)

        self.fia = FIA(out_channel, 32, 32)  # #定义FIA模块
        self.abr = ABR(out_channel)  # #定义ABR模块
        self.conv_f = nn.Conv2d(out_channel, num_classes, kernel_size=(1, 1))  # #使用FIA就要单独写master_branch
        ...
        if self.aux_branch:
            ...
            self.auxiliary_branch = nn.Sequential(
                nn.Conv2d(aux_channel, out_channel // 8, kernel_size=(3, 3), padding=(1, 1), bias=False),
                norm_layer(out_channel // 8),
                nn.ReLU(inplace=True),
                nn.Dropout2d(0.1),
                nn.Conv2d(out_channel // 8, num_classes, kernel_size=(1, 1))
            )
    ...
    def forward(self, x):
        ...
        x_aux, x = self.backbone(x)  # #可选择主分支x[30,30,320](features[17])或者辅助分支x_aux[30,30,96](feature[13])
        output = self.MVit_block1(x_aux)
        output = self.conv1(output)
        output = self.MVit_block2(output)
        output = self.conv2(output)
        output = self.master_branch(output)
        output = self.fia(output)  # #插入FIA
        output = self.abr(output)  # #插入ABR
        output = self.conv_f(output)  # #最终输出
